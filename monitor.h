...
class ProducerConsumerMonitor{
    public:
        ...
        pthread_cond_t BTCNotFull;            // Condition variable for signaling if there is 6 bitcoin in the queue
        long unsigned int capacity = 15;      
        std::queue<RequestType> brokerQueue;    //shared queue

        unsigned int inRequestQueue[RequestTypeN] = {0, 0}; //used to see what is currently in the queue
